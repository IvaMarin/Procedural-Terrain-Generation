...
public class MyPlayer3D : MonoBehaviour
{
    ...
    [Header("Teledash & Sprint Physics")]
    [Range(0f, 0.5f)]
    ...
    [Header("Ledge Vault")]
    [Range(0f, 20f)]
    [Tooltip("Velocity during a vault at which point the flip will start")]
    ...
    void FixedUpdate()
    {
        ...
        if (require_release_of_cancel_to_sprint)
        {
            ...
            if (!MyInput.sprintDash)
            { // For exiting UI and not sprinting right away 
        ...
        if (controlsOn)
        {
            ...
            else if (dash_mode == dash_mode_teleport)
            {
                ...
                */
                ...
                if (first_boxcast_hit || safety_raycast_hit)
                {
                    ...
                    else
                    {
                        ...
                        if (dash_wall_normal_angle_from_ground >= 45f)
                        {
                            ...
                            { // Hit something up the slope, so move to that.
                ...
                if (inside_a_dash_prism || doing_SpeedPod_StrongDash)
                { //temp change to if you're in a dash prism trigger
                    do_vault = false;
            ...
            if (is_wallrunning)
            {
                    ...
                    wallrun_raycast_distance + pushaway, Registry.lmc_GROUND, qti_ignore))
                {
                    ...
                    else if (dash_wall_normal_angle_from_ground > allowed_wall_slope_variance * 1.25f)
                    { // 2020-10-13 - Add a multiplier, so that the margin for falling off is more generous
            ...
            else if (dash_mode == dash_mode_prismDash)
            {
                ...
                { // Wait to hit something, then enter a vault.
                    ...
                    if (!hitReflector)
                    {
                            ...
                            Registry.lmc_GROUND, qti_ignore))
                        {
                            ...
                            if (!Is_touching_ground())
                            {
                                ...
                                if (hitInfo.collider.tag == "GripWall")
                                { // Only relevant to Core level. If hitting a gripwall, give another superdash.
                                ...
                                else
                                {
                                    ...
                                    if (superdash_hack)
                                    { // don't allow dash after superdash
                                        used_dash = true;
                                    }
                                }
            ...
            else if (holdingForward || holdingBackward || holdingLeft || holdingRight || isGliding)
            {
                ...
                else if (is_sprinting_speed_on)
                {
                    ...
                    { // No sprinting if inside a no Sprint Trigger.
                        if (wasSprintingIn_NoSprintT_ButDidntLand)
                        { // This is set when sprinting in an NST. Unset it once one the ground (It's set so that you don't get sprinting velocity while in jumping from an NST)
                            if (jump_state == jump_state_ground)
                            {
                    ...
                    if (moveX > 0.15f)
                    { // Symmetrical with below left block.
                        sprint_rotVel += dv;
                        if (sprintHeldFac < 1)
                        {
...
*/
    ...
    void jumpLogic()
    {
        ...
        { //First jump, wait for gruond or double jump
            ...
            if (!(MyInput.shortcut && myJump))
            {
                ...
                else
                {
                    ...
                    {
                        if (nextJumpVel.y > 0)
                        { // Don't apply this if falling  - not intuitive
            ...
            if (trigger_jumpstate_enter_wallrun)
            {
                ...
                //print(new_entry_vel);
                if (nextJumpVel.y > -1)
                { // Don't boost upwards if already falling
        ...
        else if (jump_state == jump_state_gliding)
        {
            ...
            if (glide_needToReachFastFallingSpeed)
            { // if not falling fast enough to glide, fall as normal
                nextJumpVel.y -= Time.fixedDeltaTime * medJumpDeaccel * C_FallDampen;
                if (nextJumpVel.y <= glide_beginToStabilize_velThreshold)
                {
                    glide_needToReachFastFallingSpeed = false;
            ...
            { // if falling fast enough to glide, begin to change velocity to reach 'gliding fall velocity'.
    ...
    void Update()
    {
        ...
        if (in_scene_initialization_phase)
        {
            ...
            else if (scene_initialization_mode == 1)
            { // Frame 2 - begin player control, or block for events
                ////if (MyEvent.scheduler_event_running) { // This is set by the master event handler
                if (false)
                { // This is set by the master event handler
            ...
            else if (scene_initialization_mode == 2)
            { // Block for events
            ...
            else if (scene_initialization_mode == 3)
            { // Extra frame to do stuff
