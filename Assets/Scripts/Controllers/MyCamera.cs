...
public class MyCamera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (wallrun_mode == 0)
        {
            ...
            { // Wallrun code has offset the player already
                wallrun_mode = 1;
        ...
        if (sprinting || player.is_wallrunning)
        {////|| hoverboard.isActive) {
        ...
        else if (Mathf.Abs(convertedMoveX) > joy_leftMovementThreshold)
        { // Slight camera rotation while moving horizontally with a controller
        ...
        if (player.IsGliding() /*|| hoverboard.isActive*/ || (sprinting && !player.just_vaulted_but_didnt_land) || player.is_wallrunning || player.dash_mode == player.dash_mode_prismDash)
        {
            ...
            {
                if (t_sincePlayerWasWallrunning <= 1.5f)
                { // Wallrun > Mid-air - slow down the speed at which the camera moves back behind the player, to avoid the camera being pushed back by the wall the player jumped off of.
        ...
        if (this_frame_sprintAddY == 0)
        {
            ...
            if (sprinting && !sprint_input_held)
            { // After releasing sprint, apply the sprintAddY to the current camera  rotation so that it doesnt' awkwardly lerp back
        ...
        if (curXOrbitAngle > xAngle_thresholdForPostOffsetRotation)
        { // from -20 to 0, rotate towards the ground. Beyond -20, lock the offset rotation there and start to tilt camera
        ...
        if (NearplaneCastAndMove(1f, 0.5f, 1f - nearplaneCastDistance, 0.5f, cam))
        {// right to left
            hitWallOnLeft = true;
