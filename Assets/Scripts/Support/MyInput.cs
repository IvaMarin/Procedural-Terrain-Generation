...
public class MyInput : MonoBehaviour
{
    ...
    public bool forceMouseCamera;
    public bool forceLeftHandedMouseControls;
    public bool forceRightHandedMouseControls;
    public float mouseSmoothing = 0.9f;
    ...
    */
    ...
    void Update()
    {
        ...
        */
        ...
        else if (mouse_visible_state == 2)
        { // Mousecam on, temporarily showing cursor
            if (Input.GetMouseButtonDown(0))
            { // Click to re-lock cursor.
                Cursor.lockState = CursorLockMode.Locked;
                mouse_visible_state = 1;
            }
            else if (!SaveManager.mouse_camera_is_active)
            { // User turns mousecam off, so show cursor.
                Cursor.lockState = CursorLockMode.None;
                mouse_visible_state = 0;
            }
        }
        ...
        if (forceMouseCamera)
        {
            forceMouseCamera = false;
            SaveManager.mouse_camera_is_active = true;
        }

        if (forceLeftHandedMouseControls)
        {
            Use_preset_lhMouse();
        }
        else if (forceRightHandedMouseControls)
        {
            Use_preset_rhMouse();
        }

        forceRightHandedMouseControls = forceLeftHandedMouseControls = false;
        ...
        float used_smoothing = mouseSmoothing;
        if (SaveManager.mouseCam_accelScale < 50f)
        {
            used_smoothing = Mathf.Lerp(1f, mouseSmoothing, SaveManager.mouseCam_accelScale / 50f);
        }
